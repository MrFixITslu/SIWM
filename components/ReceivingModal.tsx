...
const ReceivingModal: React.FC<ReceivingModalProps> = ({
      ...
      const fetchFullAsn = async () => {
        ...
        try {
          ...
          if (!fullAsn || !fullAsn.items) {
            setError(
              `Could not load item details for shipment #${shipment.id}.`
            );
            setReceivingItems([]);
            return;
          }
          const itemsToReceive: ReceivingItemState[] = fullAsn.items.map(
            (item: any) => {
  ...
  const handleItemChange = (
    index: number,
    field: 'receivedQuantity' | 'receivedSerials',
    value: string
  ) => {
    ...
    try {
      const receivedItemsPayload: ReceivedItem[] = receivingItems.map(item => {
        ...
        if (isNaN(receivedQuantity) || receivedQuantity < 0) {
          throw new Error(
            `Invalid received quantity for item '${item.itemName}'.`
          );
        }

        let receivedSerials: string[] | undefined = undefined;
        if (item.isSerialized) {
          receivedSerials = item.receivedSerials
            .split(/[\s,]+/)
            .map(s => s.trim())
            .filter(Boolean);
          if (receivedSerials.length !== receivedQuantity) {
            throw new Error(
              `Quantity (${receivedQuantity}) does not match number of serials (${receivedSerials.length}) for '${item.itemName}'. Adjust quantity or serials.`
            );
          }
        }
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Receive Items for Shipment #${shipment.id}`}
      size="2xl"
    ...
    >
      <div className="space-y-4">
        ...
        <p className="text-sm text-secondary-600 dark:text-secondary-400">
          Confirm received quantities for each item. Scan or enter serial
          numbers for serialized products. Discrepancies will be automatically
          logged.
        </p>
            ...
            receivingItems.map((item, index) => (
              ...
              >
                ...
                <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                  {item.isSerialized ? (
                    <div className="md:col-span-2">
                      ...
                      <p className="text-xs text-secondary-500 mt-1">
                        Received quantity will be automatically updated based on
                        the number of serials entered.
                      </p>
                    </div>
                  ) : (
                    <div>
                  ...
                  <div
                    className={`flex items-end ${!item.isSerialized ? '' : 'md:col-start-2'}`}
                  >
                    <div className="w-full">
                      ...
                      <input
                        ...
                        readOnly
                        disabled
                        className={`mt-1 block w-full bg-secondary-100 dark:bg-secondary-800 ${TAILWIND_INPUT_CLASSES}`}
                      />
                    </div>
                  </div>
                </div>
