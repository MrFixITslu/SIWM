...
const ReportViewerModal: React.FC<ReportViewerModalProps> = ({
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      'download',
      `${reportDefinition.id}_${new Date().toISOString().split('T')[0]}.csv`
    );
  ...
  const handleExportPDF = () => {
    alert(
      'PDF export is not yet implemented. This would typically involve a library like jsPDF.'
    );
  };
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="space-x-2 flex-shrink-0">
            ...
            <button
              ...
              disabled={reportData.length === 0}
              className="px-3 py-1.5 text-xs font-medium text-red-700 bg-red-100 hover:bg-red-200 dark:bg-red-700 dark:text-red-100 dark:hover:bg-red-600 rounded-md disabled:opacity-50"
            >
              Export PDF
            </button>
          </div>
