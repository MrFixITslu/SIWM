...
const DesktopSidebar: React.FC<DesktopSidebarProps> = ({
  ...
  const renderDesktopNavLinks = (items: NavItem[]) =>
    items.map(item => (
      ...
      >
        <item.icon
          className={`${commonIconClass} ${isOpen ? 'mr-3' : 'mr-0'}`}
        />
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-30 flex-col bg-white dark:bg-secondary-800 shadow-lg transition-all duration-300 ease-in-out hidden lg:flex ${isOpen ? 'w-64' : 'w-20'}`}
    >
      <div
        className={`flex items-center h-16 px-4 border-b border-secondary-200 dark:border-secondary-700 ${isOpen ? 'justify-between' : 'justify-center'}`}
      >
        {isOpen && (
          ...
          {user && isOpen && (
            <div className="px-0 py-3 mb-2 border-b border-secondary-200 dark:border-secondary-700">
              <div className="flex items-center">
                <UserCircleIcon
                  className={`${commonIconClass} mr-3 text-secondary-500 dark:text-secondary-400 flex-shrink-0`}
                />
      ...
      <div
        className={`p-4 border-t border-secondary-200 dark:border-secondary-700 space-y-2 ${isOpen ? '' : 'flex flex-col items-center'}`}
      >
        <button
