...
export const REPORT_DEFINITIONS: ReportDefinition[] = [
  ...
  },
  {
    ...
    columns: [
      ...
      { key: 'days_in_stock', header: 'Days In Stock', sortable: true },
      {
        ...
        header: 'Cost Price',
        sortable: true,
        render: (item: { cost_price?: number }) =>
          item.cost_price ? `$${item.cost_price.toFixed(2)}` : 'N/A',
      },
      {
        key: 'total_value',
        header: 'Total Value',
        sortable: true,
        render: (item: { quantity: number; cost_price?: number }) =>
          `$${(item.quantity * (item.cost_price || 0)).toFixed(2)}`,
      },
    ],
  },
  {
    ...
    description:
      'Critical items that have fallen below their reorder point and require immediate attention.',
    category: ReportCategory.Inventory,
    ...
    columns: [
      ...
      { key: 'reorderPoint', header: 'Reorder Point', sortable: true },
      {
        ...
        header: 'Shortfall',
        sortable: true,
        render: (item: any) => `-${item.shortfall}`,
      },
      { key: 'category', header: 'Category', sortable: true },
      { key: 'location', header: 'Location', sortable: true },
  ...
  },
  {
    ...
    description:
      'Items projected to run out of stock within 6 months based on current demand patterns and lead times.',
    ...
    columns: [
      ...
      },
      {
        ...
        sortable: true,
        render: (item: any) => item.projectedStockOutDate,
      },
      { key: 'leadTime', header: 'Lead Time (Days)', sortable: true },
      {
        ...
        header: 'Variability (%)',
        sortable: true,
        render: (item: any) => `±${item.variability}%`,
      },
      { key: 'category', header: 'Category', sortable: true },
      { key: 'location', header: 'Location', sortable: true },
    ] as ColumnDefinition<any, keyof any>[],
  ...
  },
  {
    ...
    description:
      'Track historical run rates and configuration changes for demand forecasting.',
    category: ReportCategory.AIPredictive,
    ...
    columns: [
      ...
      },
      {
        ...
        header: 'Last Updated',
        sortable: true,
        render: (item: any) => new Date(item.lastUpdated).toLocaleDateString(),
      },
      {
        key: 'dailyAverage',
