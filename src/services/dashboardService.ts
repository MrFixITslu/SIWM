...
export const dashboardService = {
  ...
  getUnacknowledgedOrdersCount: async (
    signal?: AbortSignal
  ): Promise<number> => {
    const data = await api.get<any>('/dashboard/unacknowledged-orders-count', {
      signal,
    });
    return data.count || 0;
  },
    ...
    return [
      {
        ...
        value: data.ordersAcknowledged || '0',
        icon: CheckBadgeIcon,
      },
      {
        title: 'Orders Picked',
        value: data.ordersPicked || '0',
        icon: OrderIcon,
      },
      {
        title: 'Orders Packed',
        value: data.ordersPacked || '0',
        icon: DispatchIcon,
      },
      {
        title: 'Orders Shipped',
        value: data.ordersShipped || '0',
        icon: ShipmentIcon,
      },
    ];
  },
  ...
  getInventoryFlowData: async (
    warehouseId?: number,
    signal?: AbortSignal
  ): Promise<InventoryFlowData[]> => {
    const path = warehouseId
      ? `/dashboard/inventory-flow?warehouse_id=${warehouseId}`
      : '/dashboard/inventory-flow';
    const data = await api.get<any>(path, { signal });
    return data.flowData || [];
  },

  getWarehouseCapacityData: async (
    warehouseId?: number,
    signal?: AbortSignal
  ): Promise<any[]> => {
    const path = warehouseId
      ? `/dashboard/warehouse-capacity?warehouse_id=${warehouseId}`
      : '/dashboard/warehouse-capacity';
    const data = await api.get<any>(path, { signal });
    return data.capacityData || [];
  },

  getWarehousePerformanceData: async (
    warehouseId?: number,
    signal?: AbortSignal
  ): Promise<any[]> => {
    const path = warehouseId
      ? `/dashboard/warehouse-performance?warehouse_id=${warehouseId}`
      : '/dashboard/warehouse-performance';
    const data = await api.get<any>(path, { signal });
    return data.performanceData || [];
  },

  // Customer support dashboard methods
  getSupportDashboardMetrics: async (
    signal?: AbortSignal
  ): Promise<SupportDashboardMetric> => {
  ...
  getItemsBelowReorderPoint: async (
    warehouseId?: number,
    signal?: AbortSignal
  ): Promise<any[]> => {
    const path = warehouseId
      ? `/dashboard/items-below-reorder-point?warehouse_id=${warehouseId}`
      : '/dashboard/items-below-reorder-point';
    const data = await api.get<any>(path, { signal });
    return data.items || [];
  },

  getItemsAtRiskOfStockOut: async (
    warehouseId?: number,
    signal?: AbortSignal
  ): Promise<any[]> => {
    const path = warehouseId
      ? `/dashboard/items-at-risk?warehouse_id=${warehouseId}`
      : '/dashboard/items-at-risk';
    const data = await api.get<any>(path, { signal });
    return data.items || [];
  },
  ...
  getStockAnalysis: async (
    ...
    const path = warehouseId
      ? `/dashboard/stock-analysis?warehouse_id=${warehouseId}`
  ...
  getInventoryFlowChart: async (
    warehouseId?: number,
    signal?: AbortSignal
  ): Promise<any[]> => {
    const path = warehouseId
      ? `/dashboard/inventory-flow-chart?warehouse_id=${warehouseId}`
      : '/dashboard/inventory-flow-chart';
    const data = await api.get<any>(path, { signal });
    return data.flowData || [];
  },
  ...
  syncOfflineActions: async (signal?: AbortSignal): Promise<any> => {
    const data = await api.post<any>(
      '/dashboard/sync-offline-actions',
      {},
      { signal }
    );
    return {
  ...
  subscribeToWarehouseUpdates: (warehouseId?: number) => {
    const eventSource = new EventSource(
      `/api/v1/dashboard/warehouse-updates${warehouseId ? `?warehouse_id=${warehouseId}` : ''}`
    );
    return eventSource;
  },
  ...
  getStockValueByDepartment: async (signal?: AbortSignal): Promise<any[]> => {
    const data = await api.get<any>('/dashboard/stock-value-by-department', {
      signal,
    });
  ...
  getOutOfStockItemsWithDetails: async (
    signal?: AbortSignal
  ): Promise<any[]> => {
    const data = await api.get<any>('/dashboard/out-of-stock-items', {
      signal,
    });
    return data.items || [];
  },
