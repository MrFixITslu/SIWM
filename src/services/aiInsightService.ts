...
class AiInsightService {
  ...
  private async retryWithDelay<T>(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (
          (error.isRateLimit || error.status === 429) &&
          attempt < maxRetries
        ) {
  ...
  async getStockOutForecast(
    signal?: AbortSignal
  ): Promise<StockOutRiskForecastItem[]> {
    try {
      return await this.retryWithDelay(async () => {
        ...
        const result = await api.get<StockOutRiskForecastItem[]>(
          '/ai-insights/stock-forecast',
          { signal }
        );
        ...
        console.warn(
          'Stock out forecast from backend returned non-array data:',
          result
        );
        return [];
      });
      ...
      if (error.isRateLimit || error.status === 429) {
        ...
        throw new Error(
          `Rate limit exceeded. Please wait ${error.retryAfter || 60} seconds before trying again.`
        );
      }
      ...
      if (
        ...
        throw new Error(
          'AI insights temporarily unavailable due to API quota limits. Please try again later.'
        );
      }

      console.error(
        'Failed to get stock out forecast via backend:',
        error.message
      );
    ...
    try {
      return await this.retryWithDelay(async () => {
        ...
        const result = await api.post<{ text: string }>(
          '/ai-insights/asn-delay-prediction',
          { asn },
          { signal }
        );
      ...
      console.error(
        'Failed to get ASN delay prediction via backend:',
        error.message
      );
  ...
  async getRouteOptimizationSuggestion(
    ...
    if (relevantShipments.length === 0)
      return 'No active shipments eligible for route optimization suggestions at this time.';

    try {
      return await this.retryWithDelay(async () => {
        const result = await api.post<{ text: string }>(
          '/ai-insights/route-optimization',
      ...
      if (error.isRateLimit || error.status === 429) {
        console.warn(
          'Rate limit exceeded for route optimization:',
          error.message
        );
        return `Rate limit exceeded. Please wait ${error.retryAfter || 60} seconds before trying again.`;
      }
      ...
      console.error(
        'Failed to get route optimization via backend:',
        error.message
      );
