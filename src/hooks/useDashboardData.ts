...
export const useDashboardData = (options: UseDashboardDataOptions = {}) => {
  ...
  const fetchAsnKpis = useCallback(
    async (signal: AbortSignal, forceRefresh = false) => {
      ...
      try {
        ...
        const durations = asns.map((asn: any) => {
          const created = asn.createdAt
            ? new Date(asn.createdAt).getTime()
            : null;
          ...
          return {
            landedToApprovalRequest:
              created && approvalRequest
                ? (approvalRequest - created) / 3600000
                : null,
            approvalRequestToApproved:
              approvalRequest && approved
                ? (approved - approvalRequest) / 3600000
                : null,
            approvedToPaid:
              approved && paid ? (paid - approved) / 3600000 : null,
        ...
        const kpis = {
          avgLandedToApprovalRequest: avg(
            durations.map(d => d.landedToApprovalRequest)
          ),
          avgApprovalRequestToApproved: avg(
            durations.map(d => d.approvalRequestToApproved)
          ),
  ...
  const computedValues = useMemo(() => {
    const totalItems = data.warehouseMetrics.reduce(
      (sum, warehouse) => sum + warehouse.inventory_items,
      0
    );
    const totalOrders = data.warehouseMetrics.reduce(
      (sum, warehouse) => sum + warehouse.pending_orders,
      0
    );
    const totalAlerts = data.warehouseMetrics.reduce(
      (sum, warehouse) => sum + warehouse.stock_alerts,
      0
    );
    const avgCapacity =
      data.warehouseMetrics.length > 0
        ? data.warehouseMetrics.reduce(
            (sum, warehouse) => sum + warehouse.capacity_percentage,
            0
          ) / data.warehouseMetrics.length
        : 0;
    ...
    return {
      ...
      totalAlerts,
      avgCapacity,
      criticalItems: data.itemsBelowThreshold.filter(item => item.critical)
        .length,
