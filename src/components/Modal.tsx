...
const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <FocusTrap active={isOpen}>
      ...
      >
        <div
          className={`bg-surface dark:bg-secondary-800 rounded-2xl shadow-2xl border border-border w-full ${sizeClasses[size]} transform transition-all duration-300 ease-in-out scale-100 opacity-100 animate-modal-appear focus:outline-none`}
          tabIndex={-1}
        >
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-border">
            <h3 className="text-xl font-bold text-text-primary dark:text-secondary-100 tracking-tight">
              {title}
            </h3>
            <button
              onClick={onClose}
