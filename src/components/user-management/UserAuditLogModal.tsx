...
const UserAuditLogModal: React.FC<UserAuditLogModalProps> = ({
  isOpen,
  onClose,
  user,
}) => {
        ...
        <div className="max-h-[60vh] overflow-y-auto space-y-3">
          {logs.length > 0 ? (
            logs.map(log => (
              <div
                key={log.id}
                className="p-3 bg-secondary-50 dark:bg-secondary-700/50 rounded-md text-sm"
              >
