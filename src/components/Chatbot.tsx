  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      const stream = await geminiService.sendChatMessageStream(
        ...
        messages
          .filter(
            m => m.sender === 'user' || (m.sender === 'ai' && !m.isLoading)
          )
          .slice(-10),
        ...
        setMessages(prev =>
          prev.map(msg =>
            msg.id === aiThinkingMessage.id
              ? { ...msg, text: fullResponse, isLoading: true }
              : msg
          )
        );
        scrollToBottom();
      ...
      if ((error as Error).name !== 'AbortError') {
        ...
        setMessages(prev =>
          prev.filter(msg => msg.id !== aiThinkingMessage.id)
        );
                ...
                >
                  {msg.isLoading && msg.sender === 'ai' ? (
                    <div className="flex items-center">
                      {msg.text.length === 0 && (
                        <LoadingSpinner className="w-4 h-4 mr-2 animate-spin-slow" />
                      )}
                      <p className="text-sm whitespace-pre-wrap blinking-cursor">
                        {msg.text}
                      </p>
                    </div>
                  ) : (
                  ...
                  <p
                    className={`text-xs mt-1 ${msg.sender === 'user' ? 'text-primary-200 text-right' : 'text-secondary-500 dark:text-secondary-400 text-left'}`}
                  >
