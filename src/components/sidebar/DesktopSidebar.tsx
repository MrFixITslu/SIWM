...
const DesktopSidebar: React.FC<DesktopSidebarProps> = ({
  ...
  const linkClass =
    'flex items-center px-4 py-3 rounded-lg hover:bg-primary-50 dark:hover:bg-primary-900/10 hover:text-primary-600 dark:hover:text-primary-300 transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-primary-500';
  ...
  const renderDesktopNavLinks = (items: NavItem[]) =>
    items.map(item => (
      ...
      >
        ...
        {isOpen && item.isBeta && (
          <span className="ml-auto text-xs bg-blue-100 dark:bg-blue-700 text-blue-700 dark:text-blue-200 px-1.5 py-0.5 rounded-full">
            Beta
          </span>
        )}
      </NavLink>
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-30 flex-col bg-surface dark:bg-secondary-800 shadow-card border-r border-border transition-all duration-300 ease-in-out hidden lg:flex ${isOpen ? 'w-64' : 'w-20'}`}
      aria-label="Sidebar Navigation"
    >
      <div
        className={`flex items-center h-16 px-4 border-b border-border ${isOpen ? 'justify-between' : 'justify-center'}`}
      >
        {isOpen && (
          <span className="text-xl font-extrabold text-primary-600 dark:text-primary-400 whitespace-nowrap tracking-tight font-sans">
            {APP_NAME}
          </span>
        )}
        <button
          ...
          {user && isOpen && (
            <div className="px-0 py-3 mb-2 border-b border-border">
              <div className="flex items-center">
                <UserCircleIcon
                  className={`${commonIconClass} mr-3 text-secondary-400 flex-shrink-0`}
                />
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-text-primary dark:text-secondary-100 truncate">
                    {user.name}
                  </p>
                  <p className="text-xs text-text-secondary dark:text-secondary-400 truncate">
                    {user.email}
                  </p>
                </div>
              </div>
      ...
      <div
        className={`p-4 border-t border-border space-y-2 ${isOpen ? '' : 'flex flex-col items-center'}`}
      >
        <button
        ...
        >
          {isDarkMode ? (
            <SunIconSolid
              className={`${commonIconClass} ${isOpen ? 'mr-3' : ''}`}
            />
          ) : (
            <MoonIconSolid
              className={`${commonIconClass} ${isOpen ? 'mr-3' : ''}`}
