...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
  ...
  const isDestructive = destructiveWords.some(word =>
    (confirmButtonText || '').toLowerCase().includes(word)
  );
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} size="md">
      <div className="text-center">
        <div
          className={`mx-auto flex items-center justify-center h-12 w-12 rounded-full ${isDestructive ? 'bg-red-100 dark:bg-red-800/30' : 'bg-blue-100 dark:bg-blue-800/30'}`}
        >
          <WarningIcon
            className={`h-6 w-6 ${isDestructive ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400'}`}
            aria-hidden="true"
          />
        </div>
        <div className="mt-3 text-center sm:mt-5">
          <h3
            className="text-lg leading-6 font-medium text-secondary-900 dark:text-secondary-100"
            id="modal-title"
          >
            {title}
          </h3>
