...
const BrokerFeeModal: React.FC<BrokerFeeModalProps> = ({
  isOpen,
  onClose,
  shipment,
  onActionComplete,
  ...
  const [fees, setFees] = useState<ShipmentFees>({
    duties: 0,
    shipping: 0,
    storage: 0,
  });
  ...
  const handleSubmitFees = async () => {
    ...
    try {
      if (!fees.duties && !fees.shipping && !fees.storage) {
        throw new Error(
          'At least one fee type must have a value greater than zero.'
        );
      }
  ...
  const totalFee =
    (fees.duties || 0) + (fees.shipping || 0) + (fees.storage || 0);

  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Enter Fees for Shipment #${shipment.id}`}
      size="lg"
    ...
    >
      <div className="space-y-4">
        ...
        <p className="text-sm text-secondary-600 dark:text-secondary-400">
          Submit the following fees for financial approval. Once approved, you
          will be notified to proceed with payment.
        </p>
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <input
                ...
                className={`pl-7 pr-4 ${TAILWIND_INPUT_CLASSES} w-full`}
              />
            </div>
          </div>
          <div>
