...
import {
  REPORT_DEFINITIONS,
  NOTIFICATION_TYPES as ALERT_TYPES_CONSTANT,
} from '@/constants'; // For report names and alert types
...
const TAILWIND_CHECKBOX_CLASSES =
  'h-4 w-4 text-primary-600 border-secondary-300 dark:border-secondary-500 rounded focus:ring-primary-500';
const TAILWIND_LABEL_CLASSES =
  'ml-2 block text-sm text-secondary-700 dark:text-secondary-300';
const TAILWIND_SECTION_CLASSES =
  'p-4 bg-white dark:bg-secondary-800 shadow rounded-lg';
...
> = () => {
  ...
  const availableFrequencies: ScheduledReportSubscription['frequency'][] = [
    'Daily',
    'Weekly',
    'Monthly',
  ];
  ...
  const loadPreferences = useCallback(async () => {
    ...
    try {
      ...
      const fullAlertPreferences: UserAlertPreference[] =
        availableAlertTypes.map(alertType => {
          return (
            savedAlertsMap.get(alertType) || {
              alertType,
              enabled: true,
      ...
      const fullScheduledReports: ScheduledReportSubscription[] =
  ...
  const handleScheduledReportChange = (
    reportId: string,
    field: keyof ScheduledReportSubscription,
    value: any
  ) => {
  ...
  return (
    <div className="space-y-6">
      <div className={TAILWIND_SECTION_CLASSES}>
        <h3 className="text-lg font-medium leading-6 text-secondary-900 dark:text-secondary-100 mb-4">
          Alert Notifications
        </h3>
        <div className="space-y-4">
          {alertPreferences.map(pref => (
            <div
              key={pref.alertType}
              className="p-3 border border-secondary-200 dark:border-secondary-700 rounded-md"
            >
              <div className="flex justify-between items-center mb-2">
                ...
                <label
                  htmlFor={`enable-${pref.alertType}`}
                  className="flex items-center cursor-pointer"
                >
                  <span className="text-xs mr-2 text-secondary-600 dark:text-secondary-400">
                  ...
                  <div className="relative">
                    ...
                    />
                    <div
                      className={`block w-10 h-6 rounded-full transition ${pref.enabled ? 'bg-primary-500' : 'bg-secondary-300 dark:bg-secondary-600'}`}
                    ></div>
                    <div
                      className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition transform ${pref.enabled ? 'translate-x-full' : ''}`}
                    ></div>
                  </div>
                </label>
              </div>
              {pref.enabled && (
                <fieldset>
                  <legend className="sr-only">
                    Notification channels for {pref.alertType}
                  </legend>
                  <div className="flex flex-wrap gap-x-6 gap-y-2">
                    {availableChannels.map(channel => (
                      <div key={channel} className="flex items-center">
                        ...
                        />
                        <label
                          htmlFor={`${pref.alertType}-${channel}`}
                          className={TAILWIND_LABEL_CLASSES}
                        >
                          {channel}
                        </label>
                      </div>
        ...
        <div className="space-y-4">
          {scheduledReports.map(sub => (
            ...
            >
              <div className="flex justify-between items-center mb-2">
                <span className="font-medium text-secondary-800 dark:text-secondary-200">
                  {sub.reportName || sub.reportId}
                </span>
                <label
                  htmlFor={`enable-report-${sub.reportId}`}
                  className="flex items-center cursor-pointer"
                >
                  <span className="text-xs mr-2 text-secondary-600 dark:text-secondary-400">
                  ...
                  <div className="relative">
                    ...
                    />
                    <div
                      className={`block w-10 h-6 rounded-full transition ${sub.enabled ? 'bg-primary-500' : 'bg-secondary-300 dark:bg-secondary-600'}`}
                    ></div>
                    <div
                      className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition transform ${sub.enabled ? 'translate-x-full' : ''}`}
                    ></div>
                  </div>
                </label>
              </div>
              {sub.enabled && (
                <>
                  <div className="mb-2">
                    <label
                      htmlFor={`freq-${sub.reportId}`}
                      className="block text-xs font-medium text-secondary-600 dark:text-secondary-400 mb-0.5"
                    >
                      Frequency
                    </label>
                    <select
                      ...
                      onChange={e =>
                        handleScheduledReportChange(
                          ...
                          'frequency',
                          e.target
                            .value as ScheduledReportSubscription['frequency']
                        )
                      }
                  ...
                  <fieldset>
                    <legend className="sr-only">
                      Delivery channels for {sub.reportId}
                    </legend>
                    <div className="flex flex-wrap gap-x-6 gap-y-2">
                      {availableChannels.map(channel => (
                        <div key={channel} className="flex items-center">
