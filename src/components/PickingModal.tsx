...
const PickingModal: React.FC<PickingModalProps> = ({
  ...
  const handleFileSelect = async (
    ...
    setItemStatuses(prev => ({
      ...prev,
      [itemId]: { isLoading: true, error: null },
    }));
    ...
    try {
      ...
      if (serials.length !== currentItem.quantity) {
        throw new Error(
          `Quantity mismatch. Expected ${currentItem.quantity}, but file contains ${serials.length} serials.`
        );
      }
      ...
      setItemStatuses(prev => ({
        ...prev,
        [itemId]: { isLoading: false, error: err.message },
      }));
    } finally {
      setItemStatuses(prev => ({
        ...prev,
    ...
    try {
      const updatedItems: OrderItem[] = pickingItems.map(pItem => {
        ...
        if (pItem.isSerialized) {
          pickedSerialNumbers = pItem.pickedSerialsString
            .split(/[\s,]+/)
          ...
          if (pickedSerialNumbers.length !== pItem.quantity) {
            throw new Error(
              `For item '${pItem.name}', you must pick exactly ${pItem.quantity} serial number(s). You entered ${pickedSerialNumbers.length}.`
            );
          }

          const availableSerialsUpper =
            pItem.availableSerials?.map(s => s.toUpperCase()) || [];
          for (const sn of pickedSerialNumbers) {
            if (!availableSerialsUpper.includes(sn)) {
              throw new Error(
                `Serial number '${sn}' for item '${pItem.name}' is not available in stock.`
              );
            }
          }
        }
        ...
        return {
          ...
          name: pItem.name,
          pickedSerialNumbers: pItem.isSerialized
            ? pickedSerialNumbers
            : pItem.pickedSerialNumbers,
        };
      });
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Pick Items for Order #${order.id}`}
      size="2xl"
    ...
    >
      <div className="space-y-4">
        ...
        <p className="text-sm text-secondary-600 dark:text-secondary-400">
          Fulfill the order by confirming items and scanning/entering serial
          numbers for serialized products.
        </p>
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-secondary-500">
                    SKU: {inventoryMap[item.itemId]?.sku} | Location:{' '}
                    {item.location}
                  </p>
                </div>
              ...
              {item.isSerialized && (
                <div className="mt-3">
                  <div className="flex justify-between items-center mb-1">
                    <label
                      htmlFor={`serials-${item.itemId}`}
                      className="block text-sm font-medium text-secondary-700 dark:text-secondary-300 flex items-center"
                    >
                    ...
                    <button
                      ...
                      disabled={itemStatuses[item.itemId]?.isLoading}
                      className="text-xs bg-secondary-100 hover:bg-secondary-200 dark:bg-secondary-700 dark:hover:bg-secondary-600 px-2 py-1 rounded-md flex items-center disabled:opacity-50"
                      title="Upload Serials"
                    >
                      <UploadIcon className="h-4 w-4 mr-1" /> Upload
                    </button>
                    <label
                      htmlFor={`serial-upload-${index}`}
                      className="sr-only"
                    >
                      Upload Serials
                    </label>
                    <input
                      ...
                      title="Upload Serials"
                      placeholder="Select CSV or PDF file"
                    />
                  </div>
                  <textarea
                  ...
                  {itemStatuses[item.itemId]?.isLoading && (
                  ...
                  {itemStatuses[item.itemId]?.error && (
              ...
              {!item.isSerialized && (
                <div className="mt-3 p-2 bg-secondary-50 dark:bg-secondary-700/50 rounded-md text-sm text-secondary-600 dark:text-secondary-300">
                  This is a non-serialized item. Please pick the requested
                  quantity from the location above.
                </div>
              )}
            </div>
          ))}
