...
const ReportViewerModal: React.FC<ReportViewerModalProps> = ({
  ...
  const fetchReportData = useCallback(
    async (filtersToApply: Record<string, any>) => {
      if (!reportDefinition || !reportDefinition.sampleDataKey) return;
      setIsLoading(true);
      setError(null);
      try {
        const data = await reportingService.getReportData(
          reportDefinition.sampleDataKey,
          filtersToApply
        );
        setReportData(data);
      } catch (err: any) {
        ...
        if (err.message) {
          if (err.message.toLowerCase().includes('failed to fetch')) {
            userFriendlyError =
              'Could not connect to the server to fetch report data. Please check your network or server status.';
          } else {
  ...
  const handleExportCSV = () => {
    ...
    const safeFilename = reportDefinition.id
      .replace(/[^a-z0-9]/gi, '_')
      .toLowerCase();
    link.setAttribute(
      'download',
      `${safeFilename}_${new Date().toISOString().split('T')[0]}.csv`
    );
      ...
      reportDefinition.columns.map(col => {
        ...
        if (col.render) {
          try {
            ...
            console.warn(
              `Could not render cell for PDF export for key ${String(col.key)}`
            );
          }
        }
  ...
  const renderFilters = () => {
    if (
      !Array.isArray(reportDefinition.filters) ||
    ...
    return (
      <div className="mb-4 p-4 border border-secondary-200 dark:border-secondary-700 rounded-lg bg-secondary-50 dark:bg-secondary-800/30">
        <h4 className="text-md font-semibold mb-2 text-secondary-700 dark:text-secondary-300">
          Filters
        </h4>
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Report: ${reportDefinition.name}`}
      size="full"
    >
      <div className="w-full max-w-full overflow-x-auto">
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <div className="space-x-2 flex-shrink-0">
              <button
                ...
                className="px-3 py-1.5 text-xs font-medium text-green-700 bg-green-100 hover:bg-green-200 dark:bg-green-700 dark:text-green-100 dark:hover:bg-green-600 rounded-md disabled:opacity-50"
              >
                Export CSV
              </button>
              <button
                ...
                disabled={reportData.length === 0}
                className="px-4 py-1.5 text-xs font-medium text-red-700 bg-red-100 hover:bg-red-200 dark:bg-red-700 dark:text-red-100 dark:hover:bg-red-600 rounded-md disabled:opacity-50"
              >
                Export PDF
              </button>
            </div>
          ...
          {error && (
            <div className="my-3 p-3 bg-red-100 text-red-700 rounded-md dark:bg-red-700/30 dark:text-red-300">
              {error}
            </div>
          )}
          ...
          {isLoading ? (
            ...
            <Table
              columns={
                reportDefinition.columns as ColumnDefinition<any, keyof any>[]
              }
              data={reportData}
            />
          ) : (
            !error && (
              <p className="text-center text-secondary-500 dark:text-secondary-400 py-8">
                No data available for this report or current filter selection.
              </p>
            )
          )}
