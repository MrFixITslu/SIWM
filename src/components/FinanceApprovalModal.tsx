...
interface FinanceApprovalModalProps {
  ...
  onApproveFees: (
    id: number,
    status: FeeStatus.Approved | FeeStatus.Rejected
  ) => Promise<any>;
}
...
const FinanceApprovalModal: React.FC<FinanceApprovalModalProps> = ({
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Approve Fees for Shipment #${shipment.id}`}
      size="lg"
    >
      <div className="space-y-4">
        ...
        <p className="text-sm text-secondary-600 dark:text-secondary-400">
          Review the fees submitted by broker{' '}
          <span className="font-semibold">{shipment.brokerName || 'N/A'}</span>.
        </p>
