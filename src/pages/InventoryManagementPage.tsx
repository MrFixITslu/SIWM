...
const InventoryManagementPage: React.FC = () => {
  ...
  const [selectedItemForSerials, setSelectedItemForSerials] =
    useState<InventoryItem | null>(null);
  const [showSerialNotification, setShowSerialNotification] = useState(false);
  const [showSuccessNotification, setShowSuccessNotification] = useState(false);
  const [successMessage, setSuccessMessage] = useState('');
  const [addedItemsForAsn, setAddedItemsForAsn] = useState<
    Array<{
      name: string;
      sku: string;
      quantity: number;
  ...
  const handleReturnToShipment = async () => {
    ...
    try {
      if (addedItemsForAsn.length === 0) {
        setError(
          'You must add at least one item to receive for this shipment.'
        );
        return;
      }
      ...
      if (
        ...
        if (receivedItems.some(item => !item.itemId)) {
          setError(
            'One or more items could not be matched to inventory. Please check item names and SKUs.'
          );
          return;
        }
      ...
      const addedItemsParam =
        addedItemsForAsn.length > 0
          ? `&addedItems=${encodeURIComponent(JSON.stringify(addedItemsForAsn))}`
          : '';
  ...
  const handleOpenItemModal = useCallback((item?: InventoryItem) => {
    setError(null);
    setCurrentItem(
      item || { isSerialized: false, quantity: 0, reorderPoint: 0 }
    );
  ...
  const handleInputChange = (
    ...
    if (isCheckbox && name === 'isSerialized') {
      ...
      setCurrentItem(prev => {
        const newState: Partial<InventoryItem> = {
          ...prev,
    ...
    try {
      ...
      if (existingItem && !currentItem.id) {
        ...
        const itemToSave = {
          ...
          reorderPoint: Number(
            currentItem.reorderPoint || existingItem.reorderPoint || 0
          ),
          costPrice: Number(
            currentItem.costPrice || existingItem.costPrice || 0
          ),
        };
        ...
        await inventoryService.updateInventoryItem(
          existingItem.id,
          itemToSave as InventoryItem
        );
        // Update or add to addedItemsForAsn
        if (asnId) {
          setAddedItemsForAsn(prev => {
            const idx = prev.findIndex(
              i => i.name === existingItem.name && i.sku === existingItem.sku
            );
            const updated = {
        ...
        if (
          ...
          throw new Error(
            'Name, SKU, Category, Department, and Location are required fields.'
          );
        }
        ...
        if (itemToSave.id) {
          ...
          const savedItem = await inventoryService.addInventoryItem(
            itemToSave as Omit<InventoryItem, 'id'>
          );
          // Track the added item for this ASN
          if (asnId) {
            setAddedItemsForAsn(prev => {
              const idx = prev.findIndex(
                i => i.name === savedItem.name && i.sku === savedItem.sku
              );
              const updated = {
                name: savedItem.name,
                sku: savedItem.sku,
    ...
    try {
      ...
      for (const item of items) {
        const inventoryItem = {
          ...
          reorderPoint:
            item.reorderPoint || Math.max(Math.floor(item.quantity * 0.2), 5), // Use reorder point from Excel or default
          isSerialized: false,
      ...
      setSuccessMessage(
        `Successfully uploaded ${items.length} items from Excel file!`
      );
      setShowSuccessNotification(true);
    ...
    useMemo(
      () => [
        ...
        { key: 'category', header: 'Category', sortable: true },
        {
          ...
          sortable: true,
          render: item =>
            item.isSerialized ? item.serialNumbers?.length || 0 : item.quantity,
        },
    ...
    >
      ...
      {asnId && addedItemsForAsn.length > 0 && (
        <div className="my-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg">
          <h4 className="font-semibold text-blue-700 dark:text-blue-200 mb-2">
            Items to be Received for this Shipment:
          </h4>
      ...
      <div className="mb-4 flex flex-col md:flex-row md:items-center md:space-x-4">
        ...
        <div className="mt-2 md:mt-0">
          ...
          <select
            ...
            value={agedFilter}
            onChange={e =>
              setAgedFilter(e.target.value as 'all' | 'aged' | 'non-aged')
            }
