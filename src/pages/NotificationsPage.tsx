...
import { AlertLogEntry, InventoryItem } from '@/types';
...
const NotificationsPage: React.FC = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      let userFriendlyError =
        'An unexpected error occurred while fetching notifications. Please try again.';
      if (err.message) {
        if (err.message.toLowerCase().includes('failed to fetch')) {
          userFriendlyError =
            'Could not connect to the server to fetch notifications. Please check your network or server status.';
        } else {
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {
      ...
      let userFriendlyError =
        'Failed to mark all alerts as read. Please try again.';
      if (
        err.message &&
        err.message.toLowerCase().includes('failed to fetch')
      ) {
    ...
    <PageContainer title="Notifications & Alerts Center">
      <div className="mb-6 border-b border-secondary-300 dark:border-secondary-700">
        <nav className="-mb-px flex space-x-6" aria-label="Tabs">
          ...
          >
            <BellIcon
              className={`h-5 w-5 inline mr-1.5 ${activeTab === 'log' ? 'text-primary-500' : 'text-secondary-400'}`}
            />
            Activity Log{' '}
            {unreadCount > 0 && (
              <span className="ml-1.5 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-200">
                {unreadCount} New
              </span>
            )}
          </button>
          ...
          >
            <SettingsIcon
              className={`h-5 w-5 inline mr-1.5 ${activeTab === 'preferences' ? 'text-primary-500' : 'text-secondary-400'}`}
            />
            Preferences
          </button>
        </nav>
      ...
      {error && (
        <div className="mb-4 p-3 bg-red-100 text-red-700 rounded-md dark:bg-red-700/30 dark:text-red-300">
          {error}
        </div>
      )}
          ...
          <div className="mt-3 bg-white dark:bg-secondary-800 rounded-lg shadow p-4">
            {isLoadingIncomplete ? (
              <div className="flex items-center">
                <LoadingSpinner className="w-5 h-5 text-orange-500" />
                <span className="ml-2 text-orange-700 dark:text-orange-300">
                  Loading incomplete items...
                </span>
              </div>
            ) : incompleteItems.length === 0 ? (
              <p className="text-sm text-secondary-500 dark:text-secondary-400">
                No incomplete inventory items found.
              </p>
            ) : (
              <ul className="space-y-3">
                {incompleteItems.map(item => {
                  const missingFields = [];
                  if (!item.sku || item.sku === 'not found')
                    missingFields.push('SKU');
                  if (!item.safety_stock || item.safety_stock === 0)
                    missingFields.push('Safety Stock');
                  if (!item.reorderPoint || item.reorderPoint === 0)
                    missingFields.push('Reorder Point');
                  return (
                    <li
                      key={item.id}
                      className="border-l-4 border-orange-400 bg-orange-50 dark:bg-orange-900/20 p-3 rounded flex items-center justify-between"
                    >
                      <div>
                        ...
                        <div className="text-xs text-secondary-500 dark:text-secondary-400">
                          Department: {item.department || 'N/A'} | Quantity:{' '}
                          {item.quantity}
                        </div>
                      </div>
                      <a
