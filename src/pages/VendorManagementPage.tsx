...
const VendorManagementPage: React.FC = () => {
  ...
  const handleOpenModal = (vendor?: Vendor) => {
    ...
    setCurrentVendor(
      vendor || {
        performanceScore: 80,
        products: [],
        lastCommunicationDate: new Date().toISOString().split('T')[0],
      }
    );
    setIsModalOpen(true);
  };
    ...
    try {
      ...
      if (err.message) {
        if (err.message.toLowerCase().includes('failed to fetch')) {
          userFriendlyError =
            'Could not connect to the server to save the vendor. Please check your network or server status.';
        } else {
    ...
    showDeleteConfirmation(
      async () => {
        ...
        try {
          ...
          if (err.message.toLowerCase().includes('failed to fetch')) {
            userFriendlyError =
              'Could not connect to the server to delete the vendor. Please check your network or server status.';
          } else {
            userFriendlyError = err.message;
          }
          setError(userFriendlyError);
    ...
    (vendor: Vendor) => (
      <div className="flex space-x-1">
        <button
          onClick={() =>
            navigate(
              `/incoming-shipments?supplier=${encodeURIComponent(vendor.name)}`
            )
          }
  ...
  const columns: ColumnDefinition<Vendor, keyof Vendor>[] = [
    ...
    { key: 'email', header: 'Email', sortable: true },
    {
      ...
      header: 'Perf. Score',
      render: item => renderPerformanceScore(item.performanceScore),
      sortable: true,
    },
    {
      key: 'averageLeadTime',
      ...
      render: item =>
        item.averageLeadTime ? `${item.averageLeadTime} days` : 'Not set',
      sortable: true,
    },
    { key: 'lastCommunicationDate', header: 'Last Contact', sortable: true },
    {
      ...
      render: item => item.products.join(', '),
    },
  ];
    ...
    >
      ...
      <div className="mb-4">
        <div className="relative">
          ...
          <label htmlFor="search-vendors" className="sr-only">
            Search vendors
          </label>
          <input
            id="search-vendors"
      ...
      {!isVendorLoading && filteredVendors.length === 0 && !vendorError && (
        <p className="text-center text-secondary-500 dark:text-secondary-400 py-8">
          {searchTerm
            ...
            : 'No vendors available. Try adding one!'}
        </p>
      )}
      ...
      <Modal
        ...
        title={currentVendor.id ? 'Edit Vendor' : 'Add New Vendor'}
        size="lg"
      >
        <form
          onSubmit={e => {
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <AutocompleteInput
                ...
                onChange={value =>
                  setCurrentVendor({ ...currentVendor, contactPerson: value })
                }
                options={[
                  ...new Set(
                    vendors.map(vendor => vendor.contactPerson).filter(Boolean)
                  ),
                ]}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={e =>
                  setCurrentVendor({ ...currentVendor, phone: e.target.value })
                }
                className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}
                autoComplete="tel"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={e =>
                  setCurrentVendor({
                    ...currentVendor,
                    lastCommunicationDate: e.target.value,
                  })
                }
                className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}
                autoComplete="off"
            ...
            <div>
              ...
              <input
                ...
                onChange={e =>
                  setCurrentVendor({
                    ...currentVendor,
                    averageLeadTime: parseInt(e.target.value) || undefined,
                  })
                }
                className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}
                placeholder="e.g., 14"
          ...
          <div>
            <AutocompleteInput
              ...
              options={[
                ...new Set(
                  vendors.flatMap(vendor => vendor.products).filter(Boolean)
                ),
              ]}
          ...
          <div className="flex justify-end space-x-3 pt-2">
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-secondary-700 dark:text-secondary-300 bg-secondary-100 dark:bg-secondary-600 hover:bg-secondary-200 dark:hover:bg-secondary-500 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
              disabled={isSaving}
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-4 py-2 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50"
              disabled={isSaving}
            >
              {isSaving ? (
