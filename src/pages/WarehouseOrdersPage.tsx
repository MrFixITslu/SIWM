...
const WarehouseOrdersPage: React.FC = () => {
  ...
  const { inventory: inventoryForSelect, fetchInventory: refetchInventory } =
    useInventory(); // Use global inventory state
  const [technicians, setTechnicians] = useState<UserSummary[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const [currentOrder, setCurrentOrder] = useState<CurrentOrderFormState>({
    items: [],
  });
  ...
  const {
    isModalOpen: isConfirmPickupOpen,
    confirmButtonText: pickupConfirmText,
    showConfirmation: showConfirmPickup,
    handleConfirm: handleConfirmPickupAction,
    handleClose: handleClosePickupConfirm,
  } = useConfirmationModal();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (err.message.toLowerCase().includes('failed to fetch')) {
        userFriendlyError =
          'Could not connect to the server to fetch page data. Please check your network or server status.';
      } else {
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      const validItems = currentOrder.items.filter(
        itemFS => itemFS && typeof itemFS.itemId === 'number'
      );
      ...
      if (validItems.length < currentOrder.items.length) {
        throw new Error(
          'One or more item rows are incomplete. Please select an inventory item for each row before saving.'
        );
      }

      const itemsToSave: OrderItem[] = validItems.map(itemFS => {
        if (!itemFS.itemId)
          throw new Error('An item in the order is missing its ID.'); // Should not happen due to filter
        const inventoryItem = inventoryForSelect.find(
          invItem => invItem.id === itemFS.itemId
        );
        if (!inventoryItem)
          throw new Error(`Invalid item ID ${itemFS.itemId} selected.`);
        const pickedSerials =
          itemFS.pickedSerialNumbersString
            ?.split(',')
            .map(s => s.trim())
            .filter(s => s) || [];
        ...
        if (inventoryItem.isSerialized && pickedSerials.length > 0) {
          throw new Error(
            `Serial numbers cannot be pre-assigned during order creation for: ${inventoryItem.name}. They are entered during the picking phase.`
          );
        }
        return {
      ...
      const orderPayload: Omit<WarehouseOrder, 'id' | 'createdAt'> & {
        id?: number;
      } = {
      ...
      if (currentOrder.id) {
        savedOrder = await orderService.updateOrder(
          currentOrder.id,
          orderPayload as WarehouseOrder
        );
      } else {
        savedOrder = await orderService.createOrder(
          orderPayload as Omit<WarehouseOrder, 'id' | 'createdAt'>
        );
      }
      ...
      if (err.message) {
        if (err.message.toLowerCase().includes('failed to fetch')) {
          userFriendlyError =
            'Could not connect to the server to save the order. Please check your network or server status.';
        } else {
    ...
    showConfirmation(
      async () => {
        ...
        try {
          ...
          if (err.message.toLowerCase().includes('failed to fetch')) {
            userFriendlyError =
              'Could not connect to the server to delete the order. Please check your network or server status.';
          } else {
            userFriendlyError = err.message;
          }
          setError(userFriendlyError);
        ...
        {order.items.map((oi, index) => {
          ...
          return (
            <div key={index} className="text-xs mb-1">
              ...
              {invItem?.isSerialized &&
                oi.pickedSerialNumbers &&
                oi.pickedSerialNumbers.length > 0 && (
                  <div className="flex items-center text-blue-600 dark:text-blue-400 ml-2">
                    <SerialIcon className="h-3 w-3 mr-1" /> SNs:{' '}
                    {oi.pickedSerialNumbers.join(', ')}
                  </div>
                )}
            </div>
          );
      ...
      if (activeTab === 'awaitingPickup') {
        return (
          <button
            ...
            className="flex items-center text-sm bg-purple-500 hover:bg-purple-600 text-white font-semibold px-3 py-1.5 rounded-lg shadow-sm"
            disabled={isSaving}
          >
  ...
  const dataForCurrentTab =
    activeTab === 'myOrders' ? myOrders : awaitingPickupOrders;
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 border-b border-secondary-300 dark:border-secondary-700">
        <nav className="-mb-px flex space-x-6" aria-label="Tabs">
          <button
            ...
            className={`whitespace-nowrap pb-3 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'myOrders' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-secondary-500 hover:text-secondary-700'}`}
          >
            My Active Orders
          </button>
          <button
            onClick={() => setActiveTab('awaitingPickup')}
            className={`whitespace-nowrap pb-3 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'awaitingPickup' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-secondary-500 hover:text-secondary-700'}`}
          >
            Awaiting Pickup
          </button>
        </nav>
      ...
      {!isLoading && dataForCurrentTab.length === 0 && (
        <p className="text-center text-secondary-500 dark:text-secondary-400 py-8">
          {activeTab === 'myOrders'
            ...
            : 'There are no orders awaiting pickup.'}
        </p>
      )}
      {!isLoading && dataForCurrentTab.length > 0 && (
      ...
      <ConfirmationModal
        ...
        title="Delete Warehouse Order"
        message="Are you sure you want to delete this order? This action cannot be undone."
        confirmButtonText={deleteConfirmText}
      />
      <ConfirmationModal
        isOpen={isConfirmPickupOpen}
        onClose={handleClosePickupConfirm}
        onConfirm={handleConfirmPickupAction}
        title="Confirm Order Pickup"
        message="Are you sure you have picked up all items for this order? This will update the status to 'Picked Up'."
        confirmButtonText={pickupConfirmText}
      />
      ...
      <Modal
        ...
        title={currentOrder.id ? 'Edit Warehouse Order' : 'Create New Order'}
        size="xl"
      ...
      >
        ...
        >
          ...
          <fieldset className="border border-secondary-300 dark:border-secondary-600 p-4 rounded-md">
            <legend className="text-sm font-medium text-secondary-700 dark:text-secondary-300 px-1">
              Items
            </legend>
            {currentOrder.items?.map((item, index) => {
              const selectedInvItem = inventoryForSelect.find(
                invItem => invItem.id === item.itemId
              );
              const filteredInventoryForSearch = item.itemSearchTerm
                ? inventoryForSelect.filter(
                    inv =>
                      inv.name
                        .toLowerCase()
                        .includes(item.itemSearchTerm!.toLowerCase()) ||
                      inv.sku
                        .toLowerCase()
                        .includes(item.itemSearchTerm!.toLowerCase())
                  )
              ...
              return (
                <div
                  key={index}
                  className="space-y-2 mb-3 pb-3 border-b border-secondary-200 dark:border-secondary-700 last:border-b-0 last:mb-0 last:pb-0"
                >
                  <div className="grid grid-cols-12 gap-2 items-start">
                    <div className="col-span-12 sm:col-span-5 relative">
                      <label
                        htmlFor={`item-search-${index}`}
                        className="block text-xs font-medium text-secondary-600 dark:text-secondary-400"
                      >
                        Item
                      </label>
                      <input
                        ...
                        value={
                          selectedInvItem
                            ? `${selectedInvItem.name} (${selectedInvItem.sku})`
                            : item.itemSearchTerm || ''
                        }
                        onChange={e =>
                          handleItemChange(
                        ...
                        <ul className="absolute z-10 w-full bg-white dark:bg-secondary-800 border border-secondary-300 dark:border-secondary-600 rounded-md shadow-lg max-h-48 overflow-y-auto mt-1">
                          {filteredInventoryForSearch.map(invItem => (
                            <li
                              key={invItem.id}
                              onClick={() =>
                                handleItemChange(index, 'itemId', invItem.id)
                              }
                    ...
                    <div className="col-span-10 sm:col-span-2">
                      ...
                      <input
                        ...
                        min="1"
                        max={
                          selectedInvItem && !selectedInvItem.isSerialized
            ...
            <button
              ...
              className="text-sm mt-2 text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300 font-medium flex items-center"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                name="priority"
                id="priority"
                value={currentOrder.priority || 'Medium'}
                onChange={e =>
                  setCurrentOrder({
                    ...currentOrder,
