...
const DispatchLogisticsPage: React.FC = () => {
  ...
  const [currentShipment, setCurrentShipment] =
    useState<CurrentShipmentFormState>({});
  const [currentOrderForStatus, setCurrentOrderForStatus] =
    useState<WarehouseOrder | null>(null);
  const [orderForPicking, setOrderForPicking] = useState<WarehouseOrder | null>(
    null
  );
  const [shipmentForAction, setShipmentForAction] =
  ...
  const {
    isModalOpen: isConfirmReceiptOpen,
    showConfirmation: showConfirmReceipt,
    handleConfirm: handleConfirmReceiptAction,
    handleClose: handleCloseReceiptConfirm,
  } = useConfirmationModal();

  const [isRouteOptimizationModalOpen, setIsRouteOptimizationModalOpen] =
    useState(false);
  const [confirmationModalMessage, setConfirmationModalMessage] = useState('');
  const [error, setError] = useState<string | null>(null);
  const [isSaving, setIsSaving] = useState(false);

  const [routeOptimizationSuggestion, setRouteOptimizationSuggestion] =
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (err.message.toLowerCase().includes('failed to fetch')) {
        userFriendlyError =
          'Could not connect to the server. Please check your network or server status.';
      } else {
  ...
  const handleDispatchCreate = useCallback(
    (newDispatch: OutboundShipment) => {
  ...
  const handleDispatchUpdate = useCallback(
    (updatedDispatch: OutboundShipment) => {
      setShipments(prev =>
        prev.map(s => (s.id === updatedDispatch.id ? updatedDispatch : s))
      );
      ...
      if (shipments.length > 0) {
        ...
        try {
          ...
          setRouteOptimizationSuggestion(
            'AI suggestion unavailable due to an error.'
          );
        } finally {
        ...
        setRouteOptimizationSuggestion(
          'No active shipments to analyze for route optimization.'
        );
      }
    };
    fetchAiSuggestion();
  ...
  const handleOpenShipmentModal = (order?: WarehouseOrder) => {
    ...
    if (order) {
      setCurrentShipment({
        orderId: order.id,
        itemsForDispatch: order.items.map(item => ({
          ...item,
          shippedSerialNumbersString:
            item.pickedSerialNumbers?.join(', ') || '',
        })),
        carrier: 'FedEx',
        status: 'Preparing',
  ...
  const handleSaveShipment = async () => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          'Tracking #, Destination, Est. Delivery, Dispatch Date, and Broker are required.'
        );
      }
      ...
      if (Array.isArray(currentShipment.itemsForDispatch)) {
        for (const item of currentShipment.itemsForDispatch) {
          ...
          if (invItem?.isSerialized) {
            const serials =
              item.shippedSerialNumbersString
                ?.split(',')
                .map(s => s.trim())
                .filter(Boolean) || [];
      ...
      if (currentShipment.id) {
        ...
        await dispatchService.createDispatch(
          payload as Omit<OutboundShipment, 'id'>
        );
      }
  ...
  const handleSaveStatusUpdate = async () => {
    ...
    setError(null);
    try {
      await orderService.updateOrder(currentOrderForStatus.id, {
        status: currentOrderForStatus.status,
      });
  ...
  const handleMarkAsDelivered = async (shipment: OutboundShipment) => {
    showConfirmation(
      async () => {
        try {
          await dispatchService.updateDispatch(shipment.id, {
            status: 'Delivered',
  ...
  const handleApplyOptimizedRoute = () => {
    if (
      ...
      !routeOptimizationSuggestion.toLowerCase().includes('suggestion')
    )
      return;
    alertingService
      .addAlert(
        AlertSeverity.Info,
        `AI Route Optimization applied: "${routeOptimizationSuggestion}"`,
        'Route Optimization',
        '/dispatch'
      )
      .then(() => {
        setConfirmationModalMessage(
          'AI route optimization has been logged and sent to the logistics team for implementation.'
        );
  ...
  const getOrderStatusBadge = (status: OrderStatus) => {
    const statusColors: Record<OrderStatus, string> = {
      ...
      [OrderStatus.Acknowledged]:
        'bg-cyan-100 dark:bg-cyan-700 text-cyan-800 dark:text-cyan-200',
      [OrderStatus.Picking]:
  ...
  const getShipmentStatusBadge = (status: OutboundShipment['status']) => {
    const statusClasses: Record<OutboundShipment['status'], string> = {
      ...
      Returned:
        'bg-orange-100 dark:bg-orange-700 text-orange-800 dark:text-orange-200',
    };
    return (
      <span
  ...
  const renderOrderTableActions = useCallback(
  ...
  const renderShipmentTableActions = useCallback(
    (shipment: OutboundShipment) => {
      return (
        <div className="flex space-x-2">
          {user?.role === 'Broker' &&
            shipment.feeStatus === FeeStatus.PendingSubmission && (
              <button
                onClick={() => handleOpenBrokerFeeModal(shipment)}
          ...
          {user?.role === 'Finance' &&
            shipment.feeStatus === FeeStatus.PendingApproval && (
              <button
                onClick={() => handleOpenFinanceModal(shipment)}
                className="text-green-600 hover:text-green-800 p-1"
                title="Approve Fees"
              >
                <CheckBadgeIcon className="h-5 w-5" />
          ...
          {user?.role === 'Broker' &&
            shipment.feeStatus === FeeStatus.Approved && (
              <button
                onClick={() => handleOpenPaymentModal(shipment)}
                className="text-teal-600 hover:text-teal-800 p-1"
    ...
    () =>
      warehouseOrders.filter(
        o =>
          ...
          (o.department
            .toLowerCase()
            .includes(searchTermOrders.toLowerCase()) ||
  ...
  const orderColumns: ColumnDefinition<WarehouseOrder, keyof WarehouseOrder>[] =
    [
      ...
      { key: 'department', header: 'Department' },
      {
        key: 'status',
        header: 'Status',
        render: item => getOrderStatusBadge(item.status),
      },
      {
        key: 'picker',
        header: 'Technician',
        render: item => item.picker || 'N/A',
      },
    ];

  const shipmentColumns: ColumnDefinition<
    OutboundShipment,
    keyof OutboundShipment
  >[] = [
    { key: 'id', header: 'Dispatch ID', sortable: true },
    {
      key: 'orderId',
      header: 'Order ID',
      render: item => item.orderId || 'N/A',
      sortable: true,
    },
    {
      key: 'brokerName',
      header: 'Broker',
      sortable: true,
      render: item => item.brokerName || 'N/A',
    },
    {
      key: 'fees',
      header: 'Fees (Total)',
      sortable: true,
      render: item =>
        `$${((item.fees?.duties || 0) + (item.fees?.shipping || 0) + (item.fees?.storage || 0)).toFixed(2)}`,
    },
    {
      key: 'feeStatus',
      header: 'Fee Status',
    ...
    },
    {
      ...
      render: item => getShipmentStatusBadge(item.status),
      sortable: true,
    },
  ...
  if (isLoading)
    return (
      <PageContainer title="Dispatch & Logistics">
        {' '}
        <div className="flex justify-center items-center h-64">
          {' '}
          <LoadingSpinner className="w-12 h-12 text-primary-500" />{' '}
          <p className="ml-4 text-lg">Loading dispatch data...</p>{' '}
        </div>{' '}
      </PageContainer>
    );
    ...
    <PageContainer
      ...
      actions={
        (user?.role === 'admin' || user?.role === 'manager') && (
          ...
          >
            {' '}
            <PlusIcon className="h-5 w-5 mr-2" /> Create Shipment{' '}
          </button>
        )
      }
    >
      ...
      {user && ['Warehouse', 'admin', 'manager'].includes(user.role) && (
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-secondary-800 dark:text-secondary-200 mb-3">
            Warehouse Order Processing Queue
          </h2>
          <div className="mb-4">
            <input
              type="text"
          ...
          <Table<WarehouseOrder>
            ...
            data={activeOrders}
            actions={renderOrderTableActions}
            rowClassName={item =>
              highlightedRow?.type === 'order' && highlightedRow.id === item.id
      ...
      <div>
        ...
        <Table<OutboundShipment>
          columns={shipmentColumns}
          data={filteredShipments}
          actions={renderShipmentTableActions}
          rowClassName={item =>
            highlightedRow?.type === 'shipment' && highlightedRow.id === item.id
              ? 'animate-row-highlight'
              : ''
          }
        />
      ...
      <BrokerFeeModal
        ...
        onActionComplete={() => {}}
        onSubmitFees={dispatchService.submitFees}
      />
      ...
      <PaymentConfirmationModal
        ...
        shipment={shipmentForAction}
        onActionComplete={() => {}}
        onConfirmPayment={dispatchService.confirmPayment}
      />

      <Modal
        isOpen={isStatusModalOpen}
        onClose={handleCloseStatusModal}
        title={`Update Status for Order #${currentOrderForStatus?.id}`}
      >
        <ErrorMessage message={error} />
        ...
        title={
          currentShipment.id
            ...
            : currentShipment.orderId
              ? `Create Dispatch for Order #${currentShipment.orderId}`
              : 'Create Inter-Warehouse Shipment'
        }
        size="xl"
      >
        <form
        ...
        >
          ...
          <div>
            ...
            <select
              name="brokerId"
              id="brokerId"
              value={currentShipment.brokerId || ''}
              onChange={e =>
                setCurrentShipment({
                  ...currentShipment,
                  brokerId: Number(e.target.value),
                })
              }
              required
              className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}
          ...
          <div>
            {' '}
            <label
              htmlFor="destinationAddressModal"
              className="block text-sm font-medium text-secondary-700 dark:text-secondary-300"
            >
              Destination Address
            </label>{' '}
            <textarea
              ...
              id="destinationAddressModal"
              value={currentShipment.destinationAddress || ''}
              onChange={e =>
                setCurrentShipment({
                  ...currentShipment,
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                name="carrier"
                id="carrierModal"
                value={currentShipment.carrier || 'FedEx'}
                onChange={e =>
                  setCurrentShipment({
                    ...currentShipment,
                    carrier: e.target.value as
                      | 'FedEx'
                      | 'UPS'
                      | 'DHL'
                      | 'Other',
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              {' '}
              <label
                htmlFor="estimatedDeliveryDateModal"
                className="block text-sm font-medium text-secondary-700 dark:text-secondary-300"
              >
                Est. Delivery Date
              </label>{' '}
              <input
                ...
                id="estimatedDeliveryDateModal"
                value={currentShipment.estimatedDeliveryDate || ''}
                onChange={e =>
                  setCurrentShipment({
                    ...currentShipment,
      ...
      <ConfirmationModal
        ...
        onClose={handleCloseReceiptConfirm}
        onConfirm={handleConfirmReceiptAction}
        title="Confirm Order Receipt"
        message="Are you sure you want to confirm receipt of all items for this order? This will mark the order as Completed."
      ...
      >
        ...
        <div className="flex justify-end pt-4">
          {' '}
          <button
            onClick={() => setIsRouteOptimizationModalOpen(false)}
