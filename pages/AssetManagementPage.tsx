...
const AssetManagementPage: React.FC = () => {
  ...
  const [currentMaintenanceRecord, setCurrentMaintenanceRecord] = useState<
    Partial<MaintenanceRecord>
  >({});
  const [selectedAssetForMaintenance, setSelectedAssetForMaintenance] =
    useState<WarehouseAsset | null>(null);
  ...
  const handleSaveAsset = async () => {
    ...
    try {
      if (
        ...
        throw new Error(
          'Asset Name, Type, Location, Status, and Purchase Date are required.'
        );
      }
      if (currentAsset.id) {
      ...
      if (err.message.toLowerCase().includes('failed to fetch')) {
        userFriendlyError =
          'Could not connect to the server to save the asset.';
      } else {
  ...
  const handleSaveMaintenanceRecord = async () => {
    ...
    try {
      if (
        !currentMaintenanceRecord.date ||
        !currentMaintenanceRecord.type ||
        !currentMaintenanceRecord.description
      ) {
        throw new Error(
          'Date, Type, and Description are required for maintenance record.'
        );
      }
      const recordToSave = {
      ...
      if (currentMaintenanceRecord.id) {
        const updatedRecord = await assetService.updateMaintenanceRecord(
          selectedAssetForMaintenance.id,
          currentMaintenanceRecord.id,
          recordToSave as MaintenanceRecord
        );
        setSelectedAssetLogs(prev =>
          prev.map(r => (r.id === updatedRecord.id ? updatedRecord : r))
        );
      } else {
        const newRecord = await assetService.addMaintenanceRecord(
          selectedAssetForMaintenance.id,
          recordToSave as Omit<MaintenanceRecord, 'id' | 'assetName'>
        );
      ...
      if (err.message) userFriendlyError = err.message;
      setError(userFriendlyError);
      maintenanceModalContentRef.current?.scrollTo({
        top: 0,
        behavior: 'smooth',
      });
    } finally {
  ...
  const renderMaintenanceTableActions = useCallback(
  ...
  const getAssetStatusBadge = (status: AssetStatus) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 text-xs font-medium rounded-full ${statusClasses[status] || statusClasses[AssetStatus.Decommissioned]}`}
      >
        {status}
      </span>
    );
    ...
    [
      {
        ...
        render: item => (
          <span
            className={`font-medium ${isMaintenanceOverdue(item) ? 'text-red-500 dark:text-red-400' : ''}`}
          >
            {item.name}
      ...
      { key: 'location', header: 'Location', sortable: true },
      {
        key: 'status',
        header: 'Status',
        render: item => getAssetStatusBadge(item.status),
        sortable: true,
      },
      { key: 'purchaseDate', header: 'Purchase Date', sortable: true },
      {
        key: 'nextScheduledMaintenance',
        header: 'Next Maintenance',
        sortable: true,
        render: item => item.nextScheduledMaintenance || 'N/A',
      },
    ];

  const maintenanceColumns: ColumnDefinition<
    MaintenanceRecord,
    keyof MaintenanceRecord
  >[] = [
    ...
    { key: 'description', header: 'Description', sortable: false },
    {
      ...
      header: 'Performed By',
      sortable: true,
      render: item => item.performedBy || 'N/A',
    },
    {
      key: 'cost',
      header: 'Cost',
      sortable: true,
      render: item => (item.cost ? `$${item.cost.toFixed(2)}` : 'N/A'),
    },
  ];
  ...
  return (
    ...
    >
      ...
      <ErrorMessage
        message={
          !isAssetModalOpen && !isMaintenanceModalOpen
            ? error || assetError
            : null
        }
      />
      ...
      {!isAssetLoading && filteredAssets.length === 0 && !assetError && (
        <p className="text-center text-secondary-500 dark:text-secondary-400 py-8">
          {searchTerm
            ...
            : 'No assets available. Try adding one!'}
        </p>
      )}
      ...
      <Modal
        ...
        title={currentAsset.id ? 'Edit Asset' : 'Add New Asset'}
        size="2xl"
        contentRef={assetModalContentRef}
      >
        <form
          onSubmit={e => {
            e.preventDefault();
        ...
        >
            ...
            <div>
              ...
              <select
                ...
                onChange={e =>
                  setCurrentAsset({
                    ...currentAsset,
                    assetType: e.target.value as WarehouseAssetType,
                  })
                }
                required
                className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}
              >
                ...
                {Object.values(WarehouseAssetType).map(type => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={e =>
                  setCurrentAsset({ ...currentAsset, location: e.target.value })
                }
                required
                className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}
              />{' '}
            ...
            <div>
              ...
              >
                {' '}
                {Object.values(AssetStatus).map(type => (
                  <option key={type} value={type}>
                    {type}
                  </option>
                ))}{' '}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                value={currentAsset.lastMaintenanceDate || ''}
                onChange={e =>
                  setCurrentAsset({
                    ...currentAsset,
            ...
            <div>
              ...
              <input
                ...
                value={currentAsset.nextScheduledMaintenance || ''}
                onChange={e =>
                  setCurrentAsset({
                    ...currentAsset,
          ...
          <div>
            ...
            <textarea
              ...
              onChange={e =>
                setCurrentAsset({ ...currentAsset, notes: e.target.value })
              }
              rows={3}
              className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}
            ></textarea>
      ...
      <Modal
        ...
        title={`Maintenance Log: ${selectedAssetForMaintenance?.name || ''}`}
        size="2xl"
        contentRef={maintenanceModalContentRef}
      >
        ...
        <form
          ...
          className="space-y-4 p-4 border border-secondary-200 dark:border-secondary-700 rounded-lg bg-secondary-50 dark:bg-secondary-800/30 mb-6"
        >
          <h4 className="text-lg font-medium text-secondary-800 dark:text-secondary-200">
            {currentMaintenanceRecord.id ? 'Edit Record' : 'Add New Record'}
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              {' '}
              ...
              <input
                ...
                id="maintenanceDate"
                value={currentMaintenanceRecord.date || ''}
                onChange={e =>
                  setCurrentMaintenanceRecord({
                    ...currentMaintenanceRecord,
                    date: e.target.value,
            ...
            <div>
              ...
              <select
                ...
                id="maintenanceType"
                value={currentMaintenanceRecord.type || ''}
                onChange={e =>
                  setCurrentMaintenanceRecord({
                    ...currentMaintenanceRecord,
          ...
          <div>
            ...
            <textarea
              ...
              value={currentMaintenanceRecord.description || ''}
              onChange={e =>
                setCurrentMaintenanceRecord({
                  ...currentMaintenanceRecord,
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                name="performedBy"
                id="performedBy"
                value={currentMaintenanceRecord.performedBy || ''}
                onChange={e =>
                  setCurrentMaintenanceRecord({
                    ...currentMaintenanceRecord,
            ...
            <div>
              ...
              <input
                ...
                id="maintenanceCost"
                value={currentMaintenanceRecord.cost ?? ''}
                onChange={e =>
                  setCurrentMaintenanceRecord({
                    ...currentMaintenanceRecord,
            ...
            <div>
              ...
              <input
                ...
                name="downtimeHours"
                id="downtimeHours"
                value={currentMaintenanceRecord.downtimeHours ?? ''}
                onChange={e =>
                  setCurrentMaintenanceRecord({
                    ...currentMaintenanceRecord,
                    downtimeHours: parseFloat(e.target.value),
                  })
                }
                className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}
              />{' '}
          ...
          <div className="flex justify-end space-x-3">
            <button
              type="button"
              onClick={handleClearMaintenanceForm}
              className="px-3 py-1.5 text-xs font-medium text-secondary-700 hover:bg-secondary-200 dark:hover:bg-secondary-600 rounded-md"
              disabled={isSaving}
            >
              Clear Form
            </button>
        ...
        {isMaintenanceLoading ? (
          ...
          <Table<MaintenanceRecord>
            ...
            data={selectedAssetLogs}
            actions={renderMaintenanceTableActions}
          />
        ) : (
        ...
        <div className="flex justify-end pt-4 mt-4 border-t border-secondary-200 dark:border-secondary-700">
          <button
            ...
            className="px-4 py-2 text-sm font-medium text-white bg-secondary-600 hover:bg-secondary-700 rounded-md shadow-sm"
          >
            Close Log
          </button>
        </div>
