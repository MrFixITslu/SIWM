  ...
  useEffect(() => {
    ...
    const fetchPrediction = async () => {
      if (selectedAsn) {
        ...
        try {
          const prediction = await aiInsightService.getASNDelayPrediction(
            selectedAsn,
            signal
          );
          if (!signal.aborted) {
  ...
  return (
    ...
    >
          ...
          {selectedAsn ? (
            ...
            <div className="flex items-center justify-center h-full bg-secondary-50 dark:bg-secondary-800/50 rounded-lg p-6 text-center">
              <p className="text-secondary-500 dark:text-secondary-400">
                Select a shipment to view its details and workflow actions.
              </p>
            </div>
          )}
          {selectedAsn && (
            <div className="mt-4 bg-white dark:bg-secondary-800 p-4 rounded-xl shadow-lg">
              <h4 className="text-md font-semibold text-secondary-800 dark:text-secondary-200 mb-2 flex items-center">
                <AiIcon className="h-5 w-5 mr-2 text-purple-500" /> AI Delay
                Prediction
              </h4>
              {isAiPredictionLoading ? (
                ...
                <p className="text-sm text-secondary-700 dark:text-secondary-300">
                  {aiDelayPrediction || 'No prediction available.'}
                </p>
              )}
            </div>
          )}
          ...
          <div>
            ...
            {brokers.length === 0 && (
              <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">
                No brokers available. You can create a broker user in the User
                Management section.
              </p>
            )}
          </div>
      ...
      <BrokerFeeModal
        ...
        onActionComplete={() => {
          /* Real-time update will handle it */
        }}
